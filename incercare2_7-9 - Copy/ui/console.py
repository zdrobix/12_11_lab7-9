...
class Console:
    ...
    def delete_element_ui(self):
        ...
        id = input("Enter the ID of the element you want to delete: ")
        if len(id) == 4:
            try: 
    ...
    def update_element_ui(self):
        id = input("Enter the ID of the element you want to update: ")
        if len(id) == 4:
            try:
                new_name = input("Name: ")
                new_cnp = input("CNP: ")
    ...
    def find_element_ui(self):
        id = input("Enter the ID: ")
        if len(id) == 4:
            print(self.__client_service.find_client(id))
        elif len(id) == 6:
            print(self.__movie_service.find_movie(id))
        else: print(Fore.RED + "The ID must be either 4, either 6 characters long" + Style.RESET_ALL)

    def rent_element_ui(self):
        id_client = input("Enter the ID of the client: ")
        if len(id_client) != 4:
            ...
            if len(id_movie) != 6:
                ...
                try:
                    ...
                    self.__rental_service.get_all_movies_for_client(id_client)
                    print("The movie has been rented succesfully.")
                except ValueError as e:
                    print('ERROR: ' + str(e))

    def display_rented_ui(self):
        id = input("Enter the ID of a client or movie: ")
        if len(id) == 4:
                ...
                if movies_ids != []:
                    ...
                    for id_movie in movies_ids:
                        ...
                        if movie is not None:
                            print(
                                Fore.GREEN + "Title:" + Style.RESET_ALL + ' ' + f"{movie.get_title():<20}" + Fore.GREEN + "Type:" + Style.RESET_ALL + ' ' + f"{movie.get_type():<12}", end='\n')
                        else:
                ...
                if client_ids != []:
                    ...
                    for id_client in client_ids:
                        ...
                        if client is not None:
                            print( Fore.GREEN + "Name:" + Style.RESET_ALL + ' '+ f"{client.get_name():<15}" + Fore.GREEN + "CNP:" + Style.RESET_ALL + ' ' + f"{client.get_cnp():<15}")
                            print('\n')
                        else:
    ...
    def clients_sorted_by_rented_movies(self):
        ...
        for index in range(client_number):
            ...
            if client is not None:
                print(
                    Fore.GREEN + "Name:" + Style.RESET_ALL + ' ' + f"{client.get_name():<12}" + Fore.GREEN + "CNP:" + Style.RESET_ALL + ' ' + f"{client.get_cnp():<14}" + Fore.GREEN + "Copies:" + Style.RESET_ALL + ' ' + f"{rented_copies:<3}")
    ...
    def least_rented_movies(self):
        ...
        for index in range(10):
            ...
            if movie is not None:
                print(
                    Fore.GREEN + "Title:" + Style.RESET_ALL + ' ' + f"{movie.get_title():<14}" + Fore.GREEN + "Type:" + Style.RESET_ALL + ' ' + f"{movie.get_type():<12}" + "Copies:" + Style.RESET_ALL + ' ' + f"{rented_copies:<3}",
                    end='\n')
    ...
    def random_clients_ui(self):
        nr = input ("How many random clients do you want to add?")
        nr = int(nr.strip())
        self.__client_service.rand_client(nr)
    def random_movies_ui(self):
        nr = input ("How many random movies do you want to add?")
        nr = int(nr.strip())
        self.__movie_service.rand_movie(nr)
    def random_rental_ui(self):
        nr = input("How many random rentals do you want to add?")
        nr = int(nr.strip())
    ...
    def print_movies_ui(self):
        for movie in self.__movie_service.get_all_movie():
            print( Fore.GREEN + "Title:" + Style.RESET_ALL + ' '+ f"{movie.get_title():<20}" + Fore.GREEN + "Type:" + Style.RESET_ALL + ' ' + f"{movie.get_type():<12}")
